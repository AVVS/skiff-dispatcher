...
Node.prototype.getsetnx = function (resourceId, resourceType, args, next) {
    ...
    if (typeof resourceId !== 'string' || !resourceId) {
        return next(new Error('resourceId must be a truthy string'));
    }

    if (typeof resourceType !== 'string' || !this.allowedRemoteCalls[resourceType]) {
        return next(new Error('remote call ' + resourceType + ' is not allowed'));
    }
    ...
    if (typeof operationResult !== 'undefined') {
        return next(null, operationResult);
    }
    ...
    if (currentLeader === this.id) {
        return this[resourceType](resourceId, args, next);
    }

    this._remoteCall(currentLeader, 'getsetnx', [ resourceId, resourceType, args ], callback);
};
